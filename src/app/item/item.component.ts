  ...
  implements OnInit, AfterViewInit, OnChanges, OnDestroy
{
  ...
  treeHelpMessage =
    "You can drag and drop items in the tree to move them around in the hierarchy";
  ...
  acSearch = (term$: Observable<string>): Observable<any[]> => {
    return term$.pipe(
      ...
      switchMap((term) =>
        term.length < 2
          ? []
          : this.dataSrv.searchLoincItems(term, this.loincType)
      )
    );
  };
    ...
    switch (event.eventName) {
      case "toggleExpanded":
        if (event.isExpanded) {
          this.liveAnnouncer.announce(
            `"${Util.formatNodeForDisplay(event.node)}" is expanded.`
          );
        } else {
          this.liveAnnouncer.announce(
            `"${Util.formatNodeForDisplay(event.node)}" is collapsed.`
          );
        }
        break;
      ...
      case "activate":
        ...
        setTimeout(() => {
          ...
          this.stopSpinner();
          this.liveAnnouncer.announce(
            `"${Util.formatNodeForDisplay(event.node)}" is selected`
          );
        });
        break;
  ...
  setNode(node: ITreeNode): void {
    ...
    if (
      this.focusNode &&
      this.focusNode.data &&
      (!this.focusNode.data.linkId ||
        typeof this.focusNode.data.linkId === "number")
    ) {
  ...
  cloneObject(nodeData, addCopyToName: boolean = false) {
    const clonedData = JSON.parse(JSON.stringify(nodeData));
    if (addCopyToName) {
      clonedData.text = nodeData.text + " copy";
    }
    return clonedData;
  }

  /**
   * Delete all the linkId values in a node (including it's childs)
   * @param node question to be modified
   * @returns the modified question
   */
  deleteLinksIds(node){
    if(Array.isArray(node)){
      return node.map(item => this.deleteLinksIds(item));
    } else if(node && typeof node === 'object'){
      if(node.linkId){
        console.log("deleting link id from this: ", node);
        node.linkId = undefined;
      }
      if(node.item){
        this.deleteLinksIds(node.item);
      }
    }
    return node;
  }
  ...
  onDuplicateItem(
    ...
    // get clone of the element to duplicate
    const clonedItem = this.cloneObject(nodeData, true);
    // delete all the id's
    const clonedItemWithoutIds = this.deleteLinksIds(clonedItem);

    contextNode.parent.data.item.splice(contextNode.index + 1, 0, clonedItemWithoutIds);
    this.treeComponent.treeModel.update();
  ...
  moveItem(
    ...
    position: "AFTER" | "BEFORE" | "CHILD" = "AFTER"
  ) {
  ...
  openNodeDlg(contextNode: ITreeNode, mode: "Move" | "Insert"): NgbModalRef {
    const modalRef = this.modalService.open(NodeDialogComponent, {
      ariaLabelledBy: "modal-move-title",
    });
  ...
  deleteFocusedItem() {
    ...
    nextFocusedNode = nextFocusedNode
      ? nextFocusedNode
      : this.focusNode.findPreviousSibling(true);
  ...
  addLoincItem(dialogTemplateRef): void {
    this.modalService
      .open(dialogTemplateRef, { ariaLabelledBy: "modal-basic-title" })
      .result.then(
        (autoCompResult) => {
          const subscription = this.getLoincItem(
            autoCompResult,
            this.loincType
          ).subscribe((item) => {
...
*/
  ...
  handleContextMenuOpen(open: boolean) {
    ...
    if (open) {
      this.liveAnnouncer.announce(
        `Use up or down arrow keys to navigate the menu items`
      );
    }
  }
  ...
  treeNodeFocusAnnounce(node: ITreeNode) {
    ...
    if (
      node?.data &&
      node.id !== this.treeComponent.treeModel.getActiveNode()?.id
    ) {
  ...
  logEvent(domEvent: Event) {
    console.log(
      ...
      domEvent.target instanceof HTMLElement
        ? '"' + domEvent.target.nodeName + ":" + domEvent.target.className + '"'
        : null,
      domEvent.currentTarget instanceof HTMLElement
        ? '"' +
            domEvent.currentTarget.nodeName +
            ":" +
            domEvent.currentTarget.className +
            '"'
        : null,
      domEvent instanceof FocusEvent
        ? domEvent.relatedTarget instanceof HTMLElement
          ? '"' +
            domEvent.relatedTarget.nodeName +
            ":" +
            domEvent.relatedTarget.className +
            '"'
          : null
        : null
    );
